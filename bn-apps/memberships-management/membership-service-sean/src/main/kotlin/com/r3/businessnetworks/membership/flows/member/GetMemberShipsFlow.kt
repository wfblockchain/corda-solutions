...
@InitiatingFlow
@StartableByRPC
open class GetMembershipsFlow(id: UUID?, bno : Party, private val forceRefresh : Boolean = false, private val filterOutMissingFromNetworkMap : Boolean = true) : BusinessNetworkAwareInitiatingFlow<Map<Party, StateAndRef<MembershipState<Any>>>>(id, bno) {

    @Suspendable
    override fun afterBNOIdentityVerified(bn: BusinessNetwork) : Map<Party, StateAndRef<MembershipState<Any>>> {
        val membershipService = serviceHub.cordaService(MembershipsCacheHolder::class.java)
        val cache = membershipService.cache
        val lastRefreshed = cache.getLastRefreshedTime(bn)
        ...
        val membershipsToReturn : Map<Party, StateAndRef<MembershipState<Any>>> = cache.getMemberships(bn)
...
@StartableByRPC
open class GetMembersFlow(id: UUID?, bno : Party, private val forceRefresh : Boolean = false, private val filterOutNotExisting : Boolean = true) : BusinessNetworkAwareInitiatingFlow<List<PartyAndMembershipMetadata<Any>>>(id, bno) {
    companion object {
    ...
    override fun afterBNOIdentityVerified(bn: BusinessNetwork) : List<PartyAndMembershipMetadata<Any>> {
        progressTracker.currentStep = GOING_TO_CACHE_OR_BNO
        return subFlow(GetMembershipsFlow(id, bn.bno, forceRefresh, filterOutNotExisting)).map { PartyAndMembershipMetadata(it.key, it.value.state.data.membershipMetadata) }
    }
}
